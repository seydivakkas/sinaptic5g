...
| Ölçüm | Sonuç | Durum |
|---|:---:|---|
| **Özel Dedektör mAP@0.50** | **0.9473** | ÖLÇÜLDÜ ([detector_v5_test_metrics.json](file:///d:/S%C4%B0NAPT%C4%B0C/5G%20PROJE/reports/detector_v5_test_metrics.json)) |
| **Özel Dedektör mAP@0.50:0.95** | **0.6904** | ÖLÇÜLDÜ ([detector_v5_test_metrics.json](file:///d:/S%C4%B0NAPT%C4%B0C/5G%20PROJE/reports/detector_v5_test_metrics.json)) |
| **Precision** | **0.9379** | ÖLÇÜLDÜ ([detector_v5_test_metrics.json](file:///d:/S%C4%B0NAPT%C4%B0C/5G%20PROJE/reports/detector_v5_test_metrics.json)) |
| **Recall** | **0.9333** | ÖLÇÜLDÜ ([detector_v5_test_metrics.json](file:///d:/S%C4%B0NAPT%C4%B0C/5G%20PROJE/reports/detector_v5_test_metrics.json)) |
| **F1-Score** | **0.9356** | ÖLÇÜLDÜ ([detector_v5_test_metrics.json](file:///d:/S%C4%B0NAPT%C4%B0C/5G%20PROJE/reports/detector_v5_test_metrics.json)) |
...
|---|---|---|
| **StateFarm Distracted Driver Dataset** (Kaggle) | CC BY-NC-SA 4.0 | telefonla_konusma, su_icme, esneme eylemleri |
| **Drive&Act Dataset** (TU München) | Akademik Kullanım | Kabin içi sürücü davranışları ve poz analizi |
| **AUC Distracted Driver Dataset** (AUC) | Akademik Kullanım | Sürücü dikkat dağınıklığı ve eylem sınıfları |
...
#### FTR Çevrimdışı Akış Sequence Diyagramı

```mermaid
sequenceDiagram
    participant User as Jüri / Kullanıcı
    participant Docker as FTR Docker Container
    participant Core as FTR Core
    participant Model as ONNX Detector
    participant Adapter as Competition Adapter
    participant Validator as Schema Validator
    participant Output as results.json

    User->>Docker: docker run -v input:/app/data/input -v output:/app/data/output
    Docker->>Core: main.py başlatılır
    Core->>Core: video.mp4 okunur
    Core->>Core: frame sampling + preprocessing
    Core->>Model: inference çalıştırılır
    Model-->>Core: detection sonuçları
    Core->>Core: tracking, OCR, event detection
    Core->>Adapter: yarışma formatına dönüştür
    Adapter->>Validator: JSON schema kontrolü
    Validator-->>Adapter: valid / invalid
    Adapter->>Output: /app/data/output/results.json yaz
    Output-->>User: teslim çıktısı hazır
```
...
|---|:---:|:---:|:---:|
| **mAP@0.50** | 0.9164 | **0.9473** | +3.37% |
| **mAP@0.50:0.95** | 0.6878 | **0.6904** | +0.38% |
| **Precision** | 0.9225 | **0.9379** | +1.67% |
| **Recall** | 0.8905 | **0.9333** | +4.81% |
| **F1-Score** | 0.9062 | **0.9356** | +3.24% |

### 4.3. Sınıf Bazlı Metrikler (Test Split)

| Sınıf ID | Sınıf Adı | Örnek Sayısı (N) | Precision | Recall | AP50 | AP50-95 | Yorum / Bulgular |
|:---:|---|---:|---:|---:|---:|---:|---|
| 0 | `telefonla_konusma` | 250 | 0.9886 | 0.9760 | **0.9914** | 0.6939 | Yüksek kararlılık |
| 1 | `su_icme` | 100 | 0.9883 | 1.0000 | **0.9950** | 0.8307 | Kusursuz recall |
| 2 | `arkaya_bakma` | 101 | 0.9868 | 1.0000 | **0.9950** | 0.8367 | Yüksek AP50 |
| 3 | `esneme` | 102 | 0.9440 | 0.9902 | **0.9923** | 0.6264 | Kararlı davranış |
| 4 | `sigara_icme` | 758 | 0.9132 | 0.7631 | **0.8575** | 0.5075 | Sınıf içi varyasyon yüksek |
| 5 | `emniyet_kemeri_ihlali`| 93 | 0.9193 | 0.8280 | **0.8596** | 0.5839 | Kabul kapısını (0.30) geçti |
| 6 | `teknocan` | 36 | 0.8265 | 0.9167 | **0.8973** | 0.6227 | Copy-paste ile desteklendi |
| 7 | `bilgisayar` | 26 | 0.9237 | 0.9615 | **0.9531** | 0.7208 | Dengeli ve yüksek performans |
| 8 | `license_plate` | 219 | 0.9505 | 0.9644 | **0.9841** | 0.7908 | Bounding box doğruluğudur |
...
## 8. CANLI 5G EXTENSION DİYAGRAMI

```mermaid
flowchart LR

  subgraph EDGE["Edge / Android Mobil Katman"]
    CAM["CameraX"]
    TFLITE["TFLite + NNAPI"]
    VERIFY["Number / Plate Verification"]
    UI["Mobile UI"]
  end

  subgraph NET["5G Taşıma Katmanı"]
    WEBRTC["WebRTC"]
    WS["WebSocket Metadata"]
  end

  subgraph MEC["MEC Backend"]
    BFF["FastAPI BFF"]
    MEDIA["Media Service"]
    REDIS["Redis Streams"]
  ...
  subgraph QOD["QoD Karar Motoru"]
    RISK["Risk Scoring"]
    THRESH["Calibrated Threshold"]
    ORCH["QoD Orchestrator"]
    CAMARA["CAMARA QoD Adapter"]
    LOGGER["QoD Evidence Logger"]
  end
  ...
  MEDIA --> REDIS
  REDIS --> TELEMETRY
  TELEMETRY --> RISK --> THRESH --> ORCH --> CAMARA
  ORCH --> LOGGER
  LOGGER --> UI
```
...
### Faz 1 — FTR Core’u Sabitle

Önce sadece offline çekirdek ayağa kaldırılır.

| Görev | Çıktı |
|---|---|
| `/app/data/input/video.mp4` giriş kontratı | Docker uyumlu giriş |
| `/app/data/output/results.json` çıkış kontratı | Yarışma uyumlu çıktı |
| `results.schema.json` | Şema doğrulama |
| `model_lock.json` | Model hash kanıtı |
| `test_smoke_ftr.py` | Minimum çalışma testi |

### Faz 2 — Model ve Class Contract Kilitle

| Görev | Çıktı |
|---|---|
| `class_map.yaml` oluştur | Tüm etiketler tek formata iner |
| Model SHA-256 hesapla | Yanlış model kullanımı engellenir |
| `thresholds.yaml` oluştur | Sınıf bazlı eşikler ayrılır |
| Model card yaz | Rapor için teknik savunma |

### Faz 3 — Temporal Event Sistemi Kur

Tekil frame tespiti doğrudan ihlal sayılmamalıdır.
...
#### İhlal ve Olay Kuralları

* **Kemer ihlali:** Aynı track üzerinde N frame boyunca `without_seatbelt`
- **Telefon kullanımı:** Sürücü bölgesinde telefon + temporal süre
- **Sigara:** Kabin içi smoker class + confidence threshold
- **Plaka:** OCR sonucu normalize edilmiş text
- **Hız:** Track displacement + calibration varsayımı

### Faz 4 — Evidence Paketini Otomatik Üret

`reports/` altında aşağıdaki raporlar üretilir:
- `ftr_acceptance_report.md`
- `model_lock_report.md`
- `schema_validation_report.md`
- `latency_benchmark.md`
- `dataset_manifest_report.md`
- `class_distribution_report.md`

Her çalıştırmada şu kanıtlar oluşmalıdır:

| Kanıt | Dosya |
|---|---|
| JSON validasyonu | `schema_validation_report.md` |
| Runtime süresi | `latency_benchmark.md` |
| Model hash | `model_lock_report.md` |
| Çıktı örneği | `results.json` |
| Test sonucu | `pytest_report.txt` |

### Faz 5 — Live 5G Extension’ı İzole Et

Canlı 5G tarafı FTR’ye import edilmemelidir.

- *Yanlış:* `from final_5g_extension.qod_engine import ...`
- *Doğru:*
  - `ftr_core/` (offline, bağımsız)
  - `final_5g_extension/` (demo, canlı sistem)
  - `shared/` (sadece config/model contract)
...
- [ ] Docker runtime internet istemiyor
- [ ] `/app/data/input/video.mp4` okunuyor
- [ ] `/app/data/output/results.json` oluşuyor
- [ ] `results.json` schema valid
- [ ] Model hash doğru
- [ ] Class map doğru
- [ ] Runtime benchmark raporu var
- [ ] Pytest geçiyor
- [ ] Android/MEC bağımlılığı FTR’ye sızmıyor
